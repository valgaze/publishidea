...
SpeedyBot's "magic" $ parameter is exposed to each "step" of a SpeedyBot. You can think of these steps as middleware in a traditional router

```ts
import { SpeedyBot } from "speedybot";
const Bot = new SpeedyBot();
Bot.addStep(async $ => {
 await $.send('Here is my message')
 await $.reply('Reply to original message')
...
[types.ts:140](https://github.com/valgaze/speedybot/blob/04bc6da/src/types.ts#L140)
